...
(function () {
    ...
    (function (EventType) {
        ...
        EventType["OBJECTS_HEARTBEAT"] = "ffaObjectsHeartbeat";
        EventType["HEARTBEAT"] = "ffaHeartbeat";
        EventType["UNAUTHORIZED"] = "ffaUnauthorized";
        EventType["KICK"] = "ffaKick";
        EventType["KICK_PLAYER"] = "ffaKickPlayer";
        EventType["USER_JOIN"] = "ffaUserJoin";
        EventType["NOM_KEY"] = "ffaNomKey";
        EventType["SP_NOM_KEY"] = "singlePlayerNomKey";
        EventType["HEALTH_UPDATE"] = "ffaHealthUpdate";
        EventType["DIRECTION_CHANGE_C"] = "ffaDirectionChange";
        EventType["DIRECTION_CHANGE"] = "ffaDirectionChanged";
        EventType["LOGIN_FAILED"] = "ffaLoginFailed";
        EventType["PLAYER_CREATE"] = "ffaPlayerCreate";
        EventType["PLAYER_NOMMED"] = "ffaPlayerNommed";
        EventType["PLAYER_DELETE"] = "ffaPlayerDelete";
    })(EventType || (EventType = {}));
    var Direction;
    (function (Direction) {
    ...
    socket.on(EventType.PLAYER_NOMMED, function (eventd) {
        var loser = blobs.find(function (v) { return v.owner === eventd.loser.owner; });
        var winner = blobs.find(function (v) { return v.owner === eventd.winner.owner; });
        if (!loser || !winner)
            return;
        ...
        loser.health = 100;
        displayLeaderboard();
        var nomHistoryDiv = document.getElementById("nom-hist");
        var nomEntryDiv = document.createElement("div");
        nomEntryDiv.className = "nom-hist-entry";
        var nomUser = document.createElement("span");
        var targetUser = document.createElement("span");
        nomUser.className = "nom-user nom-entry";
        nomUser.innerHTML = eventd.winner.owner + " (+" + eventd.result + ")";
        var newBRLabel = document.createElement("span");
        var newBRLabelLoser = document.createElement("span");
        newBRLabel.className = "new-br";
        newBRLabel.innerHTML = eventd.winner.br + " BR";
        var linebreakWinner = document.createElement("br");
        ...
        var linebreakLoser = document.createElement("br");
        if (!nomHistoryDiv)
            return;
    ...
    socket.on(EventType.PLAYER_DELETE, function (eventd) {
        if (details.singleplayer)
            return;
        blobs.splice(blobs.findIndex(function (v) { return v.owner === eventd; }), 1);
    });
    socket.on(EventType.LOGIN_FAILED, alert);
    socket.on(EventType.OBJECTS_HEARTBEAT, function (eventd) {
        for (var i = 0; i < eventd.walls.length; ++i) {
            var wall = new WallObject(eventd.walls[i].x, eventd.walls[i].y);
            wall.type = eventd.walls[i].type;
            objects.walls.push(wall);
        }
        objects.noNomAreas = [];
        for (var i = 0; i < eventd.noNomArea.length; ++i) {
            var area = new NoNomArea(eventd.noNomArea[i].startsAt, eventd.noNomArea[i].endsAt);
            objects.noNomAreas.push(area);
        }
    });
    socket.on(EventType.HEARTBEAT, function (eventd) {
        ...
        ownBlob.setBlob("../assets/" + eventd.blob + ".png").catch(console.log);
        blobs.push(ownBlob);
        if (details.singleplayer)
            eventd.users = [];
        var _loop_2 = function (i) {
            var currentBlob = eventd.users[i];
        ...
        for (var i = 0; i < eventd.users.length; ++i) {
            _loop_2(i);
        }
    });
    socket.on(EventType.UNAUTHORIZED, function () { return document.location.href = "/login"; });
    socket.on(EventType.KICK, function (eventd) {
        alert("You have been kicked.\nReason: " + (eventd || "-"));
        document.location.href = "/login/";
    });
    socket.on(EventType.DIRECTION_CHANGE, function (eventd) {
        if (details.singleplayer || eventd.owner === ownBlob.owner)
            return;
        var target = blobs.find(function (v) { return v.owner === eventd.owner; });
        if (!target)
            return;
    ...
    socket.on(EventType.USER_JOIN, function (eventd) {
        if (details.singleplayer ||
            eventd.owner === ownBlob.owner ||
            blobs.some(function (v) { return v.owner === eventd.owner; }))
            return;
        var newBlob = new BlobObject(eventd.br, eventd.owner);
        newBlob.directionChangeCoordinates = {
            x: eventd.x,
            y: eventd.y
        };
        ...
        newBlob
            .setBlob("../assets/" + eventd.blob + ".png")
            .then(function () { return newBlob.display(true, true); });
        blobs.push(newBlob);
    });
    socket.on(EventType.HEALTH_UPDATE, function (eventd) {
        if (details.singleplayer || typeof eventd.health !== "number")
            return;
        var target = blobs.find(function (v) { return v.owner === eventd.user; });
        if (!target)
            return;
        target.health = eventd.health;
    });
    socket.on(EventType.COORDINATE_CHANGE, function (eventd) {
        if (!ownBlob || !ownBlob.ready)
            return;
        var _loop_3 = function (i) {
            var currentBlob = eventd[i];
    ...
    function nom(attackBlob, target) {
        if (attackBlob.x < (target.x + 30) && attackBlob.x > (target.x - 30)) {
            if (attackBlob.y < (target.y + 30) && attackBlob.y > (target.y - 30)) {
                ...
                if (target.health <= 0) {
                    socket.emit(EventType.SP_NOM_KEY, { attackBlob: attackBlob, target: target }, details.id);
                    target.health = 100;
                }
            }
        }
